...
function Header() {
  return <header className="business-header"><div className="site-shell nav-wrap">
    ...
    <nav>
      ...
      <Link href="/du-an">Công trình</Link>
      <Link href="/dich-vu">Dịch vụ</Link>
      <Link href="/kien-thuc">Kiến thức</Link>
      <Link href="/gioi-thieu">Về chúng tôi</Link>
    </nav>
    <details className="mobile-nav"><summary>Menu</summary><div className="mobile-nav-panel">
      <Link href="/">Trang chủ</Link><Link href="/giai-phap">Giải pháp</Link><Link href="/san-pham">Sản phẩm</Link><Link href="/du-an">Công trình</Link><Link href="/dich-vu">Dịch vụ</Link><Link href="/kien-thuc">Kiến thức</Link><Link href="/gioi-thieu">Về chúng tôi</Link><Link href="/lien-he">Nhận tư vấn</Link>
    </div></details>
...
function Calculator() {
  ...
  return <section id="calculator" className="calculator-business"><div className="site-shell calculator-business-grid">
    <div><span className="eyebrow">SOLAR CALCULATOR</span><h2>Ước tính nhu cầu hệ thống</h2><p>Nhập hóa đơn điện trung bình để nhận một khoảng tham khảo. Đây không phải báo giá hay cam kết sản lượng; cấu hình cuối cùng cần khảo sát thực tế.</p>
      <div className="bill-value">{bill.toLocaleString('vi-VN')} <span>đ/tháng</span></div>
      <input aria-label="Hóa đơn điện trung bình mỗi tháng" type="range" min="1000000" max="30000000" step="500000" value={bill} onChange={e => setBill(Number(e.target.value))}/>
      <div className="range-labels"><span>1 triệu</span><span>30 triệu</span></div>
      <Link className="button button-light" href="/lien-he">Nhận tư vấn cấu hình →</Link>
    </div>
